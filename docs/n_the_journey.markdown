...
This is as far as the team was able to progress until a drastic turn occurred... one of the ASV's thrusters hit the hull during a small impact, which cracked the fiberglass. Unfortunately the team is currently at a standstill until this problem gets fixed and due to time the current team has been unable to determine the significance of this damage. This is a problem that will have to be diagnosed by the next UL Lafayette RoboBoat team and either a patch can be made or a new hull will be bought, or created. 
